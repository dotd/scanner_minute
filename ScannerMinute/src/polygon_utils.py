...
def _download_worker(worker_id, task_queue, result_dict, result_lock, progress,
                     ticker_task_counts):
    ...
    while True:
        ...
        try:
            ...
            with result_lock:
                ...
                ticker_done = ticker_task_counts[ticker] == ticker_task_counts.get(
                    f"_total_{ticker}", 0
                )
            logging.info(
...
def download_tickers_multithread(
    ...
    # Count tasks per ticker for tracking ticker completion
    ticker_task_counts = {}
    for task in tasks:
        ticker = task[0]
        ticker_task_counts[f"_total_{ticker}"] = (
            ticker_task_counts.get(f"_total_{ticker}", 0) + 1
        )
    ...
    progress = _ProgressTracker(len(tasks), len(tickers))

    threads = []
    for i in range(num_threads):
        t = threading.Thread(
            target=_download_worker,
            args=(i, task_queue, result_dict, result_lock, progress,
                  ticker_task_counts),
        )
        t.start()
        threads.append(t)
